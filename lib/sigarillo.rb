...
=end
  ...
  def fetch_messages(group_id, channel)
    ...
    @api.fetch().each do |message_raw|
      ...
      raw_attachments = message_raw['attachments'] || []
      message = {
        ...
        message:    {
          ...
          attachments: raw_attachments.map { |a| {
...
=end
  ...
  def to_article(message, user, ticket, channel)
    ...
    if message[:message][:body]
      ...
      unless message[:message][:attachments].nil?
        attachments = message[:message][:attachments]
        attachments.each do |attachment|
          document_result = download_file(message[:source], message[:timestamp], attachment[:file_name])
  ...
  def download_file(sender, timestamp, file_name)
    document = @api.get_file(sender, timestamp, file_name)
  end
