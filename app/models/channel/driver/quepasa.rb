...
=end
  def fetch(options, channel)
    Rails.logger.debug { 'quepasa fetch called' }
    ...
    Rails.logger.debug { "quepasa fetch started with last_seen_ts: #{last_seen_ts}" }
    ...
    Rails.logger.debug { "quepasa fetch completed, with new last_seen_ts #{new_last_seen_ts}" }
    {
      result: 'ok',
      notice: '',
    }
